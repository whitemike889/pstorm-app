...
if [ -z "$JDK" -a -s "$HOME/.PhpStorm2019.1/config/phpstorm.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.PhpStorm2019.1/config/phpstorm.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.PhpStorm2019.1/config/phpstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PhpStorm2019.1/config/phpstorm$BITS.vmoptions"
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=PhpStorm2019.1 \
